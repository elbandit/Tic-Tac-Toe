namespace Wrox.BDD.Domain
{
    public class NineSquareGrid : Grid
    {
        ...
        public GridView generate_grid_view()
        {
            ...
            for (int row = 0; row < 3; row++)
                for (int column = 0; column < 3; column++)
                    readonly_squares[row, column] = _squares[row, column] == null ? "" : _squares[row, column].value;
        ...
        {
            return contains_token_at(coordinate) ? (Square) new PlayingSquare(_squares[coordinate.X, coordinate.Y]) : new EmptyPlayingSquare();
        }        
    }
}